<nav {{--class="js-onMoveNav"--}}>
	<div class="wrap">
		...
		<div class="navMain">
            ...
            @auth
                @if(Auth::user()->status == "PRO" || Auth::user()->status == "SOA" || Auth::user()->status == "NSOA")
                    <a href="{{ url('/annonces?type=Cours') }}">@lang('Annonces Particuliers')</a>
                    <a href="{{ url('/annoncesPro?type=LookForJob') }}">@lang('Annonces Professionnels')</a>
                    <a href="{{ route('create_ad') }}">@lang('Poster une annonce')</a>
                @endif

                @if(Auth::user()->status == "PAR")
                    <a href="{{ url('/annonces?type=Cours') }}">@lang('Annonces')</a>
                @endif

                <a href="{{ url('/favoris') }}">@lang('Favoris')</a>
            @endauth
